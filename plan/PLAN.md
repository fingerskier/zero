...
**Status:** active — composite M0 **done** (contract-model, draft-1, unfrozen); M1 experimental exit **done** (`v0.1.0-local`); M2 experimental exit **done** (`v0.1.0-sdk`). Current: **M3a L2 relay process** against RELAY 0.2.2-draft.  
...
| M3a L2 relay | **in progress** — `zerodb-relay` process (handshake, persist, dual-root catch-up, resume). Merkle walk still open. |
...
**Current work package: M3a relay process** against RELAY 0.2.2-draft frames.

1. ~~Golden two-language frames~~ **done**.
2. L2 relay process (`zerodb-relay`): handshake, durable validated oplog, dual-root SYNC, cursor catch-up, reject-ack. Merkle *walk* still later.
