...
5. **`v0.1.0` Decision Log act** — this PR: names `v0.1.0` as first multi-peer secure product slice with offline catch-up (SPEC M3c exit). Two-language bar is #19 harness + #18 TS peer live to `zerodb-relay`; partition/rejoin remains M3a E3 (Rust) + TS merkle-walk. Steward creates the git tag after this merge. Formats remain draft-1 / unfrozen. **Not** format freeze. **Not** a live Rust↔TS partition executable.
