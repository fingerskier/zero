...
**Status:** active — composite M0 **done** (contract-model, draft-1, unfrozen); M1 experimental exit **done** (`v0.1.0-local`); M2 experimental exit **done** (`v0.1.0-sdk`). Current: **M3a-client** RELAY 0.2.2 LocalStore/NAPI client against `zerodb-relay`.  
...
| M3a L2 relay | **in progress** — process **done** (`9603a6c`); client (this slice) handshake + signed-op push + catch-up. Merkle walk still open. |
...
**Current work package: M3a-client** against the live L2 relay.

1. ~~Golden two-language frames~~ **done**.
2. ~~L2 relay process (`zerodb-relay`)~~ **done** (`9603a6c`).
3. RELAY 0.2.2 client in LocalStore + NAPI `connectRelay`; two-peer loopback catch-up. Merkle *walk* still later. Full EXEMPLAR E3 still later.
