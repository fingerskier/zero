...
**Date:** 2026-09-11
**Status:** M3c exited (`v0.1.0` Decision Log act @ `177e247`; steward tags). M4a-a landed main #23 @ `56a3bad`. M4a-hooks landed main #24 @ `3f81d62`. H6 first-cut landed main #25 @ `671adba`. This PR is the **H6 live WS SIGNAL fanout + role negotiation** slice. H6 is unpinned but **not closed**. Stage 0+1 landed `9903280`. E5–E8 live. M3b remainder pinned. Formats draft-1/unfrozen. **Not** M4a complete, **not** M3b exit, **not** format freeze. O4 still open.
...
| M4a-webrtc | **done** — H6 first-cut DataChannel + SIGNAL (main #25 @ `671adba`). H6 not closed |
| M4a-h6-fanout | **this PR** — live WS SIGNAL fanout + PeerId role negotiation. H6 not closed. Not M4a complete |
...
9. **M4a-h6-fanout (this PR)** — live `zerodb-relay` WebSocket SIGNAL fanout (forwarded `{sender, payload}` written to the target socket; `0x307` if missing) plus PeerId-order role negotiation (`is_handshake_server` / `isHandshakeServer`: smaller PeerId issues CHALLENGE/WELCOME). Same `AuthTranscript` / `zerodb-relay-auth-v2` — no second AUTH preimage. Evidence: `zerodb-relay/tests/signal_ws.rs`, `conformance/ts/webrtc/signal-ws.test.mjs`, role cases in `webrtc.test.mjs`. **H6 not closed** (reconnect, TURN/NAT, datastore-admission tokens, conformance profile still open). **Not** M4a complete. O4 still open.

**Pinned (do not start):**
- **perf Stage 2** — trigger: Stage 0 still scan-dominated
- **perf Stage 3** — trigger: equal/one-op-delta still full-history
- **H6 remainder** — fanout + roles are this PR; do not mark closed until reconnect, TURN/NAT, datastore admission tokens, and a conformance profile have evidence
...
- Experimental browser-peer/IDB slice grew into M4a-a adapters (#23); hooks landed #24; H6 first-cut landed #25; this PR is live SIGNAL fanout + roles on that track
