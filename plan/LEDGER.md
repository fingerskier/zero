...
| M0a | Semantic & operation kernel | in-progress (14 vectors CI-blocking both runners: HLC×4, CRDT×3, op-encoding×3 incl. multi-chunk BLAKE3, decode-neg×3, Ed25519 signature×1; OpId now cross-checked in JS via pure-JS BLAKE3; remaining: envelope §7 + BlobRef §8 vectors, equivocation/permutation suites) | VR, DQ-5..8 | L | registry merged | model suites green Rust+TS; golden vectors | kernel scope creep |
