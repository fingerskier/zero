...
| M0b | Schema IR / epochs / query profile | in-progress — O2/O3 decided (Decision Log 2026-07-16: TS authoring-canonical → IR identity-canonical, `.zerodb` dropped; minimal query subset); doc/SCHEMA.md draft (IR §2, epochs §3, migration DSL §4 + §4.1 segmented-replay model, query grammar §5). Done: IR validator + 5 negatives; **epoch-replay model both runners** (`epoch.rs`/`epoch.mjs`) + first wave EPOCH-001..004 (change_crdt replay, same-type pooling, fork lowest-order-key + descendant quarantine, EPOCH_UNKNOWN). Remaining: epoch-replay deepening (cascade quarantine, broken-chain, `reset_to`, equivocation×epoch, ep=0, depth≥3), migration-transform vectors, query-eval vectors, TS→IR compiler tool, C2 checklist | M0a ✓, DQ-10 | M | M0a vectors ✓ | epoch replay vectors incl. type change | migration DSL design |
