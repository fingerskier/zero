...
| M3b-auth-e5 | Durable AUTH membership control plane + E5 | done(`membership_grants` + `e5_membership`) | protected-store SUBSCRIBE token binds grant/principal/device/transport; requires read+sync; guessed/replayed/revoked denied; revocation cuts open sessions; SQLite reopen covered |
