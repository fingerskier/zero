...
| M3a-e2-live | E2-live CRDT matrix + E3-lite 3-peer catch-up | done(`relay_client` concurrent_crdts + three_peer + `m3a-relay.test.mjs` 3) | same `sync()` / `connectRelay`; concurrent LWW/ORSet/Flag/PNCounter; C offline while A/B write; B sqlite crash/reopen; C catch-up from R alone; merkle/op-id match; resume `received=0`. ~20–70 ops. **Not** EXEMPLAR E2 equal-ts, **not** 1000-op E3, no Merkle walk, no authz |
