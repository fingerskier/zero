...
| M3b-revoke-r1 | Revocation R1: relay `revoked` monotone + limits stated | done(this PR; `e5_membership` re-delivery cases) | Research note PR #35 (`plan/REVOCATION.md`, G1–G10). Remaining: R2a (grace window + `ts ≥ deps.ts`, founder un-revocable, self-revoke; vectors both runners) after §7 decisions; R3 "revoke implies rotate" (H10 close path). **Not** G1/G4 fixed, **not** H10 closed. |
