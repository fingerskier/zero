...
| M3c Interop + release (`v0.1.0`) | 2026-09-11 | Decision Log act (this PR); steward tags `v0.1.0` after merge. #17 SchemaEpoch n=1, #18 TS wire peer, #19 two-language harness (H9 not removed), #20 SUPPORT/UPGRADE, #21 WELCOME `protocol_version` reject (`0x102`). Two-language bar is #19 + #18 live to relay; partition/rejoin is M3a E3 (Rust) + TS merkle-walk. **Not** live Rust↔TS partition executable. Freeze still open. Not M3b exit. |
| M4a-browser-slice / wasm-events / push-driver / idb-journal | experimental shipped | `zerodb-wasm`; `memory_backend.rs`; `sync-driver.test.mjs`; `examples/browser-peer`. M4a proper is next live work. |
...
|----|------|--------|-------|
| M3c | Interop TS wire peer + release (include signed `SchemaEpoch`) | done(Decision Log this PR; tag pending steward after merge) | SPEC M3c exit. Evidence #17–#20 plus #21 WELCOME `protocol_version` reject (`0x102 VERSION_MISMATCH`). Two-language bar is #19 harness + #18 TS peer live to `zerodb-relay`. Partition/rejoin is M3a E3 (Rust) + TS merkle-walk. Formats draft-1 / unfrozen. **Not** live Rust↔TS partition executable / format freeze / M3b exit / H9 closed. |
