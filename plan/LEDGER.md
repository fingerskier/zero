...
| M3c Interop + release (`v0.1.0`) | 2026-09-11 | Decision Log act (this PR); steward tags `v0.1.0` after merge. #17 SchemaEpoch n=1, #18 TS wire peer, #19 two-language harness (H9 not removed), #20 SUPPORT/UPGRADE, #21 WELCOME `protocol_version` reject (`0x102`). Freeze still open. Not M3b exit. |
...
|----|------|--------|-------|
| M3c | Interop TS wire peer + release (include signed `SchemaEpoch`) | done(Decision Log this PR; tag pending steward after merge) | SPEC M3c exit. Evidence #17–#20 plus #21 WELCOME `protocol_version` reject (`0x102 VERSION_MISMATCH`). Formats draft-1 / unfrozen. **Not** format freeze / M3b exit / H9 closed. |
