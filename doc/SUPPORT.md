...
| WebRTC / H6 (`conformance/ts/webrtc/*.test.mjs` + `zerodb-relay` `signal` / `signal_ws`) | `ubuntu-latest`, Node 22 + built `zerodb-relay` | Live WS SIGNAL fanout + PeerId roles + fake ordered DataChannel; not wrtc. H6 not closed. |
...
- **H6 closed / M4a complete.** First-cut SIGNAL + DataChannel peer protocol is in `conformance/ts/webrtc/` (this PR). Role negotiation, admission tokens, reconnect, TURN, and a conformance profile are still open. Do not claim H6 closed or M4a complete.
