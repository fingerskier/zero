...
| `recv(r)` | Reject with `DRIFT_EXCEEDED` if `r.p > wall + max_drift_ms`. This is the **clock-advance** reject only — it MUST NOT be used as an excuse to drop a well-formed signed op. Else `p = max(wall, latest.p, r.p)`; base counter = max of the counters of whichever of {latest, r} tie `p` (absent if wall alone is max ⇒ `l = 0`); `l = base + 1`, on overflow `p += 1, l = 0`. Result strictly > both `latest` and `r` (I-5). |

**H1 op acceptance (peer-side, every honest peer):** an otherwise valid signed member operation whose `ts.p > wall + max_drift_ms` MUST enter the AUTH.md §6 quarantine with named outcome `CLOCK_DRIFT`. It MUST NOT materialize (so it cannot silently win LWW) and MUST NOT advance `latest`. When `ts.p ≤ wall + max_drift_ms` the same peer MUST release the op and apply it; `recv` then runs (I-5). Recovery is deterministic: every peer that holds the op applies it as soon as its own wall is inside the bound, and they converge (I-1). Overflow of the shared buffer drops oldest with `CLOCK_DRIFT_OVERFLOW`. A device that issued the op under its own far-future wall (locally accepted) still carries the original signed timestamp; receivers quarantine until the window resolves. Relays persist well-formed member ops — clock skew is not an admission reject — so two honest peers cannot be split by relay drop vs peer hold. `0x302 CLOCK_DRIFT` remains a transport MAY-refuse; a relay that refuses is not a catch-up source for that op, and the author resubmits after the window.
