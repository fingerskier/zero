...
| Date | Decision |
|------|----------|
| 2026-08-14 | **M3a transcript review fixes.** Claimed `HELLO.peer_id` is consumed and MUST equal `BLAKE3(pk)` (`RELAY-HELLO-003` valid-sig / wrong-id → `0x201`). `SYNC_RESPONSE` required root is direction-dependent (peer: `accepted_root`; relay: `validated_root`). Vectors carry ordered `{type, request_id, payload}` frames; both runners validate type codes, directions, required fields, and `request_id` correlation. Required corpus **115**. |
