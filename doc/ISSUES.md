...
| Date | Decision |
|------|----------|
| 2026-08-17 | **M3b AUTH membership + executable E5 (experimental).** Relay SQLite now persists `membership_grants`; `Relay::upsert_grant` / `revoke_grant` are the control-plane surface. Provisioning the first grant fail-closes that datastore: SUBSCRIBE requires the AUTH §3.3 token with `read` + `sync`, verifies cert/grant/signature/expiry/revocation, and binds token device to the HELLO/AUTH transport PeerId. OPS, SYNC, Merkle, and DELTA enforce the admitted grant; revocation cuts already-open sessions. `e5_membership` executes guessed-id denial, intended-member sharing, cross-device replay denial, revocation denial, and SQLite grant/revoke reopen. **Not claimed:** peer-side full §4 op authorization/E7 remainder, E8 clock quarantine, E2E/E6, handshake/TLS/resource hardening, M3b exit, format freeze. |
| 2026-08-16 | **M3b-sig — relay operation admission (experimental).** `zerodb_core::relay::admit_experimental_op` is now the persist gate: unsigned / bad-sig / tampered-body → `REJECT/SIG`; `wire.ds` ≠ frame datastore → `REJECT/AUTHZ`; author MUST equal `BLAKE3(author_pk)`; claimed OpId MUST equal KERNEL §4.4 preimage; Ed25519 over the OpId. Transport sender MAY differ from author. Evidence: core `admit_tests`, `zerodb-relay/tests/m3b_admission.rs`, existing `relay_client` including full E3. **Not claimed:** AUTH membership/E5, E7 remainder (colluding relay + peer-side), E8 clock, E2E/E6, format freeze, M3b exit. |
| 2026-08-15 | **M3a exit — durable convergence (experimental).** Negotiated `merkle-walk-v1` freezes the relay op-set snapshot at `SYNC_RESPONSE`, publishes the sorted active-bucket manifest, walks only mismatched canonical tree nodes/leaves, computes the leaf OpId difference, and returns full operations in bounded `DELTA_BATCH` frames. Legacy cursor catch-up remains for clients that do not negotiate the capability. `full_exemplar_e3_1000_ops_hard_crash_and_relay_only_catchup` executes the specified 1,000-write three-peer partition: C receives no live traffic, B hard-aborts and reopens mid-window, C catches up from R alone, all accepted roots match, and resume delivers zero effects. M3a is closed; M3b security is next. **Not claimed:** membership/authz, relay signature enforcement, E2E encryption, format freeze. |
| 2026-08-15 | **M3a-e2-live / E3-lite (experimental).** Existing RELAY 0.2.2 client (`LocalStore::sync` / NAPI `connectRelay`) now has live-loop evidence: concurrent LWW + ORSet add-wins + Flag enable-wins + PNCounter sum through an in-process relay and a live `zerodb-relay` process; 3-peer C-offline catch-up (~20–70 ops) with B sqlite **clean close/reopen** (not `e1_kill_clock` process death), C sync from R alone, matching merkle / op-id sets, resume `received=0`. **Not** claimed: EXEMPLAR E2 equal-ts/equivocation, EXEMPLAR E3 at 1000 ops, Merkle walk, AUTH membership (M3b), format freeze, M3a exit. |
| 2026-08-14 | **M3a-client (experimental).** LocalStore speaks RELAY 0.2.2-draft as a peer: HELLO/AUTH, submit signed ops (WireOp JSON in a `wire` field plus `op_id`/`author`/HLC), SYNC catch-up with resume cursor. Empty store adopts the remote datastore via `import_bundle`. NAPI `connectRelay(url, datastore?)` drives the same session over binary WebSocket. Two-peer loopback E2E (`relay_client` + `m3a-relay.test.mjs`). **Not** claimed: EXEMPLAR E3, Merkle walk, AUTH membership (M3b), format freeze. Relay still does not verify op signatures (M3b). |
| 2026-08-14 | **M3a relay process (experimental).** `zerodb-relay` speaks RELAY 0.2.2-draft envelopes: HELLO/CHALLENGE/AUTH/WELCOME, OPS/OP_ACK, SYNC catch-up, SUBSCRIBE. Durable SQLite validated oplog; `validated_root` only (no accepted-root equality claim). E3-lite: offline peer catch-up from the relay alone. **Not** claimed: EXEMPLAR E3 at 1000 ops, Merkle walk, AUTH membership (M3b), format freeze. |
| 2026-08-14 | **M3a transcript review fixes.** Claimed `HELLO.peer_id` is consumed and MUST equal `BLAKE3(pk)` (`RELAY-HELLO-003` valid-sig / wrong-id → `0x201`). SYNC required root is direction-dependent (peer: `accepted_root`; relay: `validated_root`; peers MUST NOT invent a relay validated root). Six `relay-transcript` vectors carry ordered `{type, request_id, payload, cbor_hex}` frames; both runners validate type codes, directions, required fields, `request_id` correlation, and encoded envelope bytes. Required corpus **115**. |
