...
| Date | Decision |
|------|----------|
| 2026-08-15 | **M3a-e2-live / E3-lite (experimental).** Existing RELAY 0.2.2 client (`LocalStore::sync` / NAPI `connectRelay`) now has live-loop evidence: concurrent LWW + ORSet add-wins + Flag enable-wins + PNCounter sum through an in-process relay and a live `zerodb-relay` process; 3-peer C-offline catch-up (~20–70 ops) with B sqlite close/reopen, C sync from R alone, matching merkle / op-id sets, resume `received=0`. **Not** claimed: EXEMPLAR E2 equal-ts/equivocation, EXEMPLAR E3 at 1000 ops, Merkle walk, AUTH membership (M3b), format freeze, M3a exit. |
