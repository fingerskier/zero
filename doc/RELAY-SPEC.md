...
Relay operators SHOULD limit concurrent connections per `PeerId` (RECOMMENDED: 3). This implementation enforces 3: a fourth AUTH from the same `PeerId` is `ERROR` `0x304` `TOO_MANY_CONNECTIONS` (fatal) and the session is closed. It also enforces, per listener (`ListenConfig` / binary flags): a **global connection cap** (`--max-connections`, default 1024 — an over-cap socket gets `ERROR` `0x304` `TOO_MANY_CONNECTIONS` fatal after the upgrade and never holds a slot); a **handshake deadline** (`--handshake-timeout-secs`, default 10 — TLS + WebSocket upgrade + HELLO/AUTH must complete, else `ERROR` `0x100` `HANDSHAKE_TIMEOUT` fatal; a raw TCP client that never upgrades is dropped at the same deadline); an **idle timeout** (`--idle-timeout-secs`, default 300, `0` disables — no inbound frame or WebSocket ping for that long is `GOODBYE` reason `0` message `IDLE_TIMEOUT`; §4.6 `PING` and WebSocket pings both reset it); and a **pre-decode message ceiling** (`MAX_FRAME_BYTES`) applied by the WebSocket layer before any buffering, answered with `ERROR` `0x303` `PAYLOAD_TOO_LARGE` fatal. IP-based rate limiting and DDoS mitigation are transport-level defenses outside this protocol, RECOMMENDED for production deployments.
...
- Impersonate a peer **to the relay's own auth layer** (challenge-response). Caveat: the forwarded `SIGNAL.sender` field is relay-asserted — signaling identity is not end-to-end authenticated. DataChannel AUTH proves the HELLO *client* to the handshake server (`AuthTranscript` / `zerodb-relay-auth-v2`) and, via `HELLO.channel_binding` (§14.2), that the client is on the *same DTLS association* the server sees — a relay that terminates DTLS on both legs and bridges bytes fails AUTH. CHALLENGE/WELCOME stay unsigned. In-protocol mutual auth / handshake-server identity is H5 (still open), not an H6 reopen.
...
- Handshake is the same HELLO / `zerodb-relay-auth-v2` / WELCOME as §4.1 (`AuthTranscript`). Optional `HELLO.datastore` is in the AUTH transcript when present (omit when absent so no-ds vectors stay identical). `HELLO.cursor` when `resume-cursor` is on is still not in the transcript. A signaling MITM that rewrites the offered datastore MUST fail AUTH (`0x201`) before OPS mix graphs. A populated peer bound to datastore A MUST fail closed with `AUTH_WRONG_DATASTORE` when the other side offers B; an empty store may adopt. AUTH is the client-role transcript only; the handshake server does not sign CHALLENGE/WELCOME (H5 — in-protocol mutual auth still open).
- **DTLS channel binding (H5 slice).** The client MUST send `HELLO.channel_binding` = `BLAKE3("zerodb-dc-binding-v1" ‖ min(fp) ‖ max(fp))` over the two SHA-256 DTLS certificate fingerprints (`a=fingerprint:sha-256` in its local and remote SDP) and it is in the AUTH transcript (same `AuthTranscript` hello map; omitted when absent, so relay-WebSocket and no-binding vectors stay byte-identical). The handshake server derives the value from its **own** local/remote SDP, requires the HELLO claim to equal it, and binds its own derivation into the transcript. Missing, foreign, or present-but-invalid MUST fail AUTH (`0x201`) before CHALLENGE — no nonce, no OPS. A signaling MITM that terminates DTLS on each leg therefore fails even if it forwards AUTH unchanged. Non-`sha-256` fingerprints fail closed. Registry `domain_separation.dc_channel_binding`; fixture `H6-AUTH-004`. The reference DataChannel entrypoints (`conformance/ts/webrtc/peer.mjs`) require the binding — a caller must supply the peer connection or a precomputed value; an unbound handshake is only reachable through an explicit test-only opt-out. This binds the client to the association; it does not authenticate the handshake server (rest of H5).
