...
**Status:** M0 contracts, M1 local core, M2 Node SDK, M3a durable convergence, and **M3c** (`v0.1.0` Decision Log act @ `177e247`) are complete. M3b remainder is **pinned** (E5–E8 live; **not** M3b exit). M4a-a (WASM + IDB/OPFS) landed main #23. M4a-hooks landed main #24. **H6 is closed** (Decision Log 2026-09-12; evidence #25–#28) — shared peer protocol over DataChannel. Protocol tests use a fake ordered DataChannel — not `wrtc`, no hosted TURN (TURN/NAT is a deployment choice; SIGNAL already carries opaque ICE). Signaling identity is still relay-asserted until DC AUTH. **Not** M4a complete — M4a stays open on its own platform criteria (real-browser WebRTC DataChannel path — tests use a fake channel; direct/relay parity; browser restart/offline tests); E10 is M4b. O4 is **pinned** (Decision Log 2026-09-12; out of the M4a gate, CI ceiling 300 KiB gzip). *All formats draft-1, unfrozen until an explicit Decision Log freeze names a versioned profile.*
...
| [WebRTC / H6](conformance/ts/webrtc/) | Fake ordered DataChannel + live WS SIGNAL + PeerId roles + reconnect/admission + HELLO.datastore in AUTH — H6 closed (protocol). Not M4a complete. No TURN. |
...
**H6 is closed** (Decision Log 2026-09-12). Current work is the M4a remainder (M4a stays open on its own platform criteria (real-browser WebRTC DataChannel path — tests use a fake channel; direct/relay parity; browser restart/offline tests); E10 is M4b). Pinned: O4 (2026-09-12), Stage 2/3, H10 open (leftovers implemented, not closed), M3b remainder. All formats remain draft-1 and unfrozen. Do not claim M4a complete, M3b exit, format freeze, or retag `v0.1.0`.
