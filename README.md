...
**Status:** M0 contracts, M1 local core (`v0.1.0-local`), M2 Node SDK (`v0.1.0-sdk`), M3a durable convergence, and **M3c** (`v0.1.0`, tagged @ `177e247`) are complete. M3b remainder is **pinned** (E5–E8 live; transcript AUTH, session limits, transport hardening + in-process TLS landed; **not** M3b exit). M4a is in progress: WASM + IDB/OPFS, React hooks, and the **H6** direct-P2P protocol (closed 2026-09-12, DTLS channel-bound) have landed over an in-process fake DataChannel; the real-browser DataChannel path, direct/relay parity, and browser restart/offline tests remain, so **not** M4a complete. O4 (WASM size) is pinned with a 300 KiB gzip CI ceiling. *All formats draft-1, unfrozen until an explicit Decision Log freeze names a versioned profile.*
...
Working conventions are in [AGENTS.md](AGENTS.md); the live action list is [plan/PLAN.md](plan/PLAN.md) §5 (M4a exit path). Pinned: O4, perf Stage 2/3, H5 remainder, M3b remainder. All formats remain draft-1 and unfrozen. Do not claim M4a complete, M3b exit, H5 closed, format freeze, or retag `v0.1.0`.
