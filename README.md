...
Offline-first, peer-to-peer, CRDT-powered **property graph database** — a successor to GunDB that keeps its zero-config, local-first developer experience while fixing its correctness failures (wall-clock conflict resolution, no oplog, JS-only core, LWW-everything).

**Status:** specification draft, pre-implementation. Current work is **Milestone 0** — stabilizing the operation format, schema epochs, Merkle sync, and trust-model contracts before any code freezes a wire or persistent format.
...
Start with [ISSUES.md](doc/ISSUES.md). The Critical (C-series) issues must be resolved before any format freezes — design proposals against those IDs are the highest-value contributions right now.
